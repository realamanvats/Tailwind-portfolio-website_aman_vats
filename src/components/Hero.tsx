...
const Hero = () => {
  ...
  const skills = [
    {
      ...
      items: [
        { name: "C++", level: 90, icon: <Zap className="w-4 h-4" /> },
        { name: "JavaScript", level: 85, icon: <Code className="w-4 h-4" /> },
        { name: "C", level: 80, icon: <Cpu className="w-4 h-4" /> }
      ]
    },
    {
      category: "Web Technologies",
      ...
      items: [
        { name: "React", level: 85, icon: <Laptop className="w-4 h-4" /> },
        { name: "HTML/CSS", level: 90, icon: <Eye className="w-4 h-4" /> },
        { name: "Tailwind CSS", level: 85, icon: <Sparkles className="w-4 h-4" /> }
      ]
    },
    {
      category: "Salesforce & Cloud",
      ...
      items: [
        { name: "Salesforce Flow", level: 80, icon: <Workflow className="w-4 h-4" /> },
        { name: "Object Management", level: 75, icon: <Database className="w-4 h-4" /> },
        { name: "Reports & Dashboards", level: 75, icon: <BarChart3 className="w-4 h-4" /> }
      ]
    }
  ...
  const projects = [
    {
      title: "Web-Based Sorting Visualizer",
      description: "Interactive educational tool that brings sorting algorithms to life through smooth animations and real-time visualization",
      technologies: ["React", "JavaScript", "Vite", "Tailwind CSS", "Framer Motion"],
      status: "Completed",
      icon: <Brain className="w-8 h-8" />,
      features: ["Real-time visualization", "Multiple algorithms", "Educational animations"],
      link: "#"
    },
    {
      title: "AI Resume Analyzer",
      description: "AI-powered tool that analyzes resumes and provides ATS scores with personalized feedback for job seekers",
      technologies: ["React", "JavaScript", "Puter.js", "Zustand", "Tailwind CSS"],
      status: "Completed",
      icon: <FileText className="w-8 h-8" />,
      features: ["ATS Score Calculation", "AI Analysis", "Personalized Feedback"],
      link: "#"
    },
    {
      title: "Salesforce Recruiting App",
      description: "Functional recruiting application built with Salesforce declarative tools for efficient hiring processes",
      technologies: ["Salesforce Flow", "Lightning", "Object Management", "Reports"],
      status: "Completed",
      icon: <Users className="w-8 h-8" />,
      features: ["Declarative Development", "Process Automation", "Custom Dashboards"],
      link: "#"
    }
  ];
  ...
  const education = [
    {
      ...
      period: "2022-Present",
      score: "CGPA: 8.11",
      courses: ["Data Structures & Algorithms", "Database Management", "Operating Systems", "Object-Oriented Programming"],
  ...
  // Fun Facts
  const funFacts = [
    { icon: <Coffee className="w-6 h-6" />, text: "Coffee consumed while coding", value: "∞ cups", color: "from-orange-500 to-red-500" },
    { icon: <Code className="w-6 h-6" />, text: "Lines of code written", value: "50K+", color: "from-green-500 to-blue-500" },
    { icon: <Rocket className="w-6 h-6" />, text: "Projects completed", value: "10+", color: "from-purple-500 to-pink-500" },
    { icon: <Heart className="w-6 h-6" />, text: "Passion for learning", value: "100%", color: "from-red-500 to-pink-500" }
  ];
  ...
  const themeClasses = darkMode 
    ? {
        ...
        text: 'text-white',
        card: 'bg-slate-800/50 backdrop-blur-sm border-white/10',
  ...
  return (
    <div className={`min-h-screen ${themeClasses.bg} ${themeClasses.text} overflow-hidden transition-colors duration-500`}>
      {/* Floating Particles */}
      <div className="fixed inset-0 overflow-hidden pointer-events-none">
        {[...Array(30)].map((_, i) => (
          <motion.div
            key={i}
            className={`absolute rounded-full bg-gradient-to-r ${darkMode ? 'from-purple-400/10 to-pink-400/10' : 'from-blue-400/20 to-purple-400/20'}`}
            style={{
              width: Math.random() * 80 + 20,
              height: Math.random() * 80 + 20,
              left: `${Math.random() * 100}%`,
      ...
      {/* Hero Section */}
      <section id="home" className="relative min-h-screen w-full flex items-center justify-center pt-20">
        <div className="relative z-10 container mx-auto px-4">
          ...
          >
            <div className="flex flex-col items-center">
              ...
              <motion.button
                onClick={() => scrollToSection('journey')}
                className="w-10 h-10 border-2 border-purple-400 rounded-full flex items-center justify-center hover:bg-purple-400/20 transition-colors"
        ...
        <div className="max-w-6xl mx-auto px-4">
          ...
          >
            {funFacts.map((fact, index) => (
              <motion.div
                key={index}
                className={`bg-gradient-to-br ${fact.color} rounded-2xl p-6 text-center backdrop-blur-sm border border-white/10 hover:border-purple-400/50 transition-all duration-300`}
              ...
              >
                ...
                <div className="text-white/80 text-sm">{fact.text}</div>
              </motion.div>
            ))}
          </motion.div>
        </div>
        ...
        <div className="max-w-6xl mx-auto px-4">
          ...
          >
            ...
            <p className="text-xl text-gray-300 max-w-2xl mx-auto">
              From learning to building, here's my path in technology
            </p>
          </motion.div>
              ...
              {activeTab === 'experience' && (
                <div className="space-y-6">
                  {experiences.map((exp, index) => (
                    <ExperienceCard key={index} experience={exp} index={index} theme={themeClasses} />
                  ))}
                </div>
              )}
              ...
              {activeTab === 'achievements' && (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  {achievements.map((achievement, index) => (
                    <AchievementCard key={index} achievement={achievement} index={index} theme={themeClasses} />
                  ))}
                </div>
              )}
              ...
              {activeTab === 'projects' && (
                <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
                  {projects.map((project, index) => (
                    <ProjectCard key={index} project={project} index={index} theme={themeClasses} />
                  ))}
                </div>
              )}
              ...
              {activeTab === 'education' && (
                <div className="space-y-6">
                  {education.map((edu, index) => (
                    <EducationCard key={index} education={edu} index={index} theme={themeClasses} />
                  ))}
                </div>
              )}
            </motion.div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            {skills.map((skillCategory, categoryIndex) => (
              <motion.div
                ...
                className={`${themeClasses.card} rounded-2xl p-6 border backdrop-blur-sm`}
              ...
              >
                <div className="flex items-center mb-6">
                  <div className="p-3 rounded-xl bg-gradient-to-r from-purple-600 to-pink-600 text-white mr-4">
                    {skillCategory.icon}
                  </div>
      ...
      <motion.button
        ...
        className="fixed top-24 right-4 z-50 p-3 rounded-full bg-white/5 backdrop-blur-sm border border-white/10 text-purple-400 hover:text-white transition-colors"
        whileHover={{ scale: 1.1, rotate: 15 }}
        whileTap={{ scale: 0.9 }}
      >
        {darkMode ? <Lightbulb size={24} /> : <Lightbulb size={24} />}
      </motion.button>
    </div>
  );
};
...
const ExperienceCard = ({ experience, index, theme }) => (
  <motion.div
    ...
    transition={{ duration: 0.6, delay: index * 0.1 }}
    viewport={{ once: true }}
    className={`${theme.card} rounded-2xl p-6 border backdrop-blur-sm hover:border-purple-500/30 transition-all duration-300 group`}
  >
...
const AchievementCard = ({ achievement, index, theme }) => (
  <motion.div
    ...
    transition={{ duration: 0.6, delay: index * 0.1 }}
    viewport={{ once: true }}
    className={`${theme.card} rounded-2xl p-6 border backdrop-blur-sm hover:border-pink-500/30 transition-all duration-300 group`}
  >
    <div className="flex items-start justify-between mb-4">
      <div className="flex items-center space-x-3">
...
const ProjectCard = ({ project, index, theme }) => (
  <motion.div
    ...
    transition={{ duration: 0.6, delay: index * 0.1 }}
    viewport={{ once: true }}
    className={`${theme.card} rounded-2xl p-6 border backdrop-blur-sm hover:border-green-500/30 transition-all duration-300 group`}
  >
    <div className="flex items-start justify-between mb-4">
      <div className="flex items-center space-x-3">
        ...
        <div>
          <h3 className="text-xl font-bold group-hover:text-green-300 transition-colors">
            {project.title}
          </h3>
...
const EducationCard = ({ education, index, theme }) => (
  <motion.div
    initial={{ opacity: 0, x: -30 }}
    whileInView={{ opacity: 1, x: 0 }}
    transition={{ duration: 0.6, delay: index * 0.1 }}
    viewport={{ once: true }}
    className={`${theme.card} rounded-2xl p-6 border backdrop-blur-sm hover:border-blue-500/30 transition-all duration-300 group`}
  >
    <div className="flex items-start justify-between mb-4">
      <div className="flex items-center space-x-3">
